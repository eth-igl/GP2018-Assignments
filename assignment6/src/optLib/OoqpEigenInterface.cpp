...
#include "ooqpei_numerical_comparisons.h"
...
bool OoqpEigenInterface::solve(const SparseMatrixd& Q,
						  const VectorXd& c,
						  ...
						  VectorXd& x)
{
//	printProblemFormulation(Q, c, A, b, C, d, f, l, u);
...
//  }
  ...
  VectorXd gamma(nx), phi(nx);		//x>=l and x<=u
  gamma.setConstant(0); phi.setConstant(0);
  VectorXd y(my);				//Ax=b
  y.setConstant(0);
  VectorXd lambda(mz), pi(mz), z(mz);	//Cx>=d, Cx<=f, and z = lambda - pi
...
void OoqpEigenInterface::generateLimits(
	...
	VectorXd& lowerLimit, VectorXd& upperLimit,
...
void OoqpEigenInterface::printSolution(int& status, VectorXd& x)
{
  if (status == 0)
  {
    cout << "-------------------------------" << endl;
